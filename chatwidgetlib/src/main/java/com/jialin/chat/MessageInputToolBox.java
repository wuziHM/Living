...
public class MessageInputToolBox extends RelativeLayout {
	...
	public void setFunctionData(List<Option> functionData) {
		...
		for (int x = 0; x < (functionData.size() % 8 == 0 ? functionData.size() / 8 : (functionData.size() / 8) + 1); x++) {
			...
			FunctionAdapter functionAdapter = new FunctionAdapter(context, functionData.subList(x * 8, ((x + 1) * 8) > functionData.size() ? functionData.size() : ((x + 1) * 8)));
...
//		PagerAdapter facePagerAdapter = new FacePageFragment.FacePagerAdapter(functionGridViewList);
		fuctionViewPager.setAdapter(new PagerAdapter() {
			...
			public void destroyItem(View arg0, int arg1, Object arg2) {
				Log.e("fuctionViewPager","destroyItem: " + arg0 +  "  " + arg1 + "  " + arg2);
